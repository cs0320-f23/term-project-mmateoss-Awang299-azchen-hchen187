...
import edu.brown.cs.student.main.server.audioRecognition.audioRecords.audioHandlerRecord.AudioHandlerPostObj;
...
public class AudioTextHandler implements Route {
  ...
  public Object handle(Request req, Response res) {
    ...
    try{
      ...
      responseMap.put("Result", "Error");
      responseMap.put("Error Message", "Please make a post request of a json object with"
          + " your base64 encoded audio as the value to the key data");
      return moshi.adapter(Map.class).toJson(responseMap);
    } catch(URISyntaxException e){
      responseMap.put("Result", "Error");
      responseMap.put("Error Message", "Malformed URI for API calls, please ensure"
      ...
      responseMap.put("Result", "Error");
      responseMap.put("Error Message", "Audio either had no words, model could not find words in"
          + " the audio");
      return moshi.adapter(Map.class).toJson(responseMap);
    } catch (Exception e){
      responseMap.put("Result", "Error");
