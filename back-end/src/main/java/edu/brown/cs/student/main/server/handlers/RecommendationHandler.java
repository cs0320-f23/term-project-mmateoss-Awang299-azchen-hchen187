...
/**
 * This is the RecommendationHandler class. It takes care of everything that has
 * to do with the
...
public class RecommendationHandler implements Route {
   ...
   * @return returns what is mapped to the server. Usually the broadband
   *         percentage, times info is
   *         accessed, and other helpful things, but if an error occurs that is
   *         returned.
   */
  @Override
  ...
  /**
   * 
   * @param maxRecommendations- the maximum number of recommendations to be
   *                            returned
   * @param minRecommendations- the minimum number of recommendations to be
   *                            returned
   ...
   * @return Recommednation record containing the recommednations in a
   *         List<TrackProps>
   * @throws Exception - Thrown when maxAttempts exceeded
   */
  private Recommendation RecommendationAlgorithm(int maxRecommendations, int minRecommendations, int maxAttempts,
      String[] allNames, String variability) throws Exception {
    // making sure at least 1 recommendation is returned
    int attempts = 0;
    Recommendation rec = new Recommendation(new ArrayList<>(), new ArrayList<>());
    List<TrackProps> filteredTracks = new ArrayList<TrackProps>(); // Ensures our Recommendation's order
    Set<TrackProps> setTracks = new HashSet<TrackProps>(); // Ensures only unique songs recommended by Spotify counted
    while (filteredTracks.size() < minRecommendations) {
      // Return an error after 10 unsuccessful attempts
      if (attempts == maxAttempts) {
        throw new Exception("Recommendation cannot be built from the passed in "
