...
public class RecordTests {
  ...
  /**
   * Method that tests that our Recommendation Records create objects with the correct fields and
   * values.
   */
  @Test
  public void testRecommendationRecords() throws Exception {
    ...
    Assert.assertEquals(
        rec.tracks().get(0).album().images().get(0).url(),
        "https://i.scdn.co/image/ab67616d0000b273df306c07c8648c0e955cb94a");
    Assert.assertEquals(rec.tracks().get(0).album().images().get(0).height(), 640);
  }

  /**
   * Method that tests that our Features Records create objects with the correct fields and values.
   */
  @Test
  ...
  /**
   * Method that tests that our Search Records create objects with the correct fields and values.
   */
  @Test
  public void testSearchRecords() throws Exception {
    ...
    Assert.assertEquals(song.tracks().items().get(0).uri(), "spotify:track:1fXiYSWmkKJfRKMegCkI11");
    Assert.assertEquals(song.tracks().items().get(1).popularity(), 44);
    Assert.assertEquals(
        song.tracks().items().get(0).preview_url(),
        "https://p.scdn.co/mp3"
            + "-preview/4d2f72a69bc3619e6e854912fd1275b6efbcc10c?cid=8199ced0f4aa4ceba1efbcedd7b6878a");
    Assert.assertEquals(
        song.tracks().items().get(0).album().images().get(0).url(),
        "https://i.scdn.co/image/ab67616d0000b2736e2407383e952808a0602b0d");
    Assert.assertEquals(song.tracks().items().get(0).artists().get(0).name(), "Mumford & Sons");
  }
