...
/**
 * Class using mocked data used to ensure that we can make and parse the api call to get text from
 * audio.
 */
public class mockedAudioDataTest {

  /**
   * Method that tests we can get the text of an audio file correctly and that we are making the Api
   * call correctly.
   *
  ...
  public void TestApi() throws URISyntaxException, IOException, InterruptedException {
    ...
    Assert.assertEquals(
        parsedAudio.results().channels().get(0).alternatives().get(0).transcript(),
